...
Node prompts, design status, decisions, open work, references and pin prompts are part of the model, but they do not have to be stored inline in the `.mod.blu` file. A node may use `promptRepo` to point to a model-owned markdown file that contains this information.
...
- [Relevant local document](../../../docs/relevant-document.md) — Why the
  document is relevant to this node.
- [Relevant external specification](https://example.com/specification) — How
  the specification constrains or informs this node.
...
The `## Node` section scopes five node-level subsections:
...
- `### References` records local documents, external files and web resources
  that are intentionally relevant to this node. Use Markdown links followed
  by a concise explanation of their relevance. Relative links resolve from
  the prompt repository file, following normal Markdown behavior. References
  on the root node apply to the whole application; references on other nodes
  remain scoped to those responsibilities.
...
Readers must continue to accept the legacy format in which text appears directly under `## Node`; that text is the node prompt. Writers save the structured format with `Prompt`, `Status`, `Decisions`, `Open` and `References` subsections, including empty subsections, so every editable node-level text has a stable title. Only links deliberately placed in `References` form the model-owned reference list; links elsewhere remain ordinary explanatory content. Tools should preserve the Markdown content and may report missing local targets or unreachable web resources without removing references automatically. If a file cannot be read or parsed, tools may fail silently and leave existing prompts unchanged.
