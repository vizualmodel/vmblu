...
__export(index_exports, {
  createVmbluMcpHttpHandler: () => createVmbluMcpHttpHandler,
  createVmbluMcpServer: () => createVmbluMcpServer,
  serveVmbluMcpStdio: () => serveVmbluMcpStdio,
  startConfiguredVmbluMcpInterfaces: () => startConfiguredVmbluMcpInterfaces,
  startVmbluMcpHttpServer: () => startVmbluMcpHttpServer
});
...
function createVmbluMcpServer({ broker, profileId, name = "vmblu-application", version = "1.0.0" } = {}) {
  ...
  const server = new import_server.McpServer({ name, version });
...
function serveVmbluMcpStdio(options = {}) {
  ...
  assertBrokerProfile(broker, profileId);
  return (0, import_stdio.serveStdio)(
...
function createVmbluMcpHttpHandler(options = {}) {
  ...
  assertBrokerProfile(broker, profileId);
  const handler = (0, import_server.createMcpHandler)(() => createVmbluMcpServer(options), {
...
async function startVmbluMcpHttpServer(options = {}) {
  ...
  const nodeHandler = (0, import_node.toNodeHandler)(fetchHandler, { onerror: options.onerror });
...
function registerApplicationTool(server, broker, profileId, tool) {
  var _a, _b, _c;
  const inputSchema = (0, import_server.fromJsonSchema)(((_a = tool.input) == null ? void 0 : _a.schema) ?? { type: "object" });
  const outputSchema = ((_c = (_b = tool.output) == null ? void 0 : _b.schema) == null ? void 0 : _c.type) === "object" ? (0, import_server.fromJsonSchema)(tool.output.schema) : void 0;
...
function registerProbeTool(server, broker, profileId, probe) {
  var _a;
  const outputSchema = ((_a = probe.schema) == null ? void 0 : _a.type) === "object" ? (0, import_server.fromJsonSchema)(probe.schema) : void 0;
  server.registerTool(`probe.${probe.id}`, {
    title: probe.title || `Read ${probe.id}`,
    description: probe.description || `Read vmblu probe ${probe.id}.`,
    inputSchema: (0, import_server.fromJsonSchema)(probe.argsSchema ?? { type: "object" }),
    ...outputSchema ? { outputSchema } : {},
...
function registerEventTool(server, broker, profileId, event) {
  var _a;
  const outputSchema = ((_a = event.schema) == null ? void 0 : _a.type) === "object" ? (0, import_server.fromJsonSchema)(event.schema) : void 0;
  server.registerTool(`event.wait.${event.id}`, {
    ...
    inputSchema: (0, import_server.fromJsonSchema)({
      type: "object",
...
function makeAuthentication(authentication = null) {
  ...
  const resourceMetadataUrl = (0, import_server.getOAuthProtectedResourceMetadataUrl)(resourceServerUrl).toString();
  const gate = (0, import_server.requireBearerAuth)({
  ...
  return {
    mode,
    gate,
    metadata: /* @__PURE__ */ __name((request) => (0, import_server.oauthMetadataResponse)(request, metadataOptions), "metadata")
  };
}
