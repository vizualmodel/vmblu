...
handleRawPaths(imports, raw) {
    ...
    const adjustPath = (userPath, isLink) => {
        const absolutePath = (isLink && ! userPath?.length) ? oldRef : Path.absolute(userPath, oldRef)
        return Path.relative(absolutePath, newRef)     
    }
    ...
    for(const rawNode of raw.root.nodes) {
        ...
        if (rawNode.kind === 'dock') {
            if (oldRef !== newRef) {
                ...
                const absolutePath = (! link.path?.length) ? oldRef : Path.absolute(link.path, oldRef)
                link.path = Path.relative(absolutePath, newRef)  
