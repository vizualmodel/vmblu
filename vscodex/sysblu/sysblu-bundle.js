...
var root_1$7 = from_html(`<i class="material-icons-outlined open svelte-16f3fxd">description</i>`);
var root_2$5 = from_html(`<button class="add-reference svelte-16f3fxd" type="button" title="Add" aria-label="Add"><i class="material-icons-outlined open svelte-16f3fxd" aria-hidden="true">add_circle</i></button>`);
var root_3$3 = from_html(`<div class="right-icons svelte-16f3fxd"><i class="material-icons-outlined trash svelte-16f3fxd">delete</i></div>`);
var root$7 = from_html(`<div><div class="hdr svelte-16f3fxd"><div class="left-icons svelte-16f3fxd"><i class="material-icons-outlined cancel svelte-16f3fxd">cancel</i> <i class="material-icons-outlined check svelte-16f3fxd">check_circle</i> <!> <!></div> <h1> </h1> <!></div> <!></div>`);
...
var root_2$4 = from_html(`<p class="svelte-1tx7ah8">No project references yet. Use + to add a reference.</p>`);
var root_4 = from_html(`<option> </option>`);
var root_3$2 = from_html(`<div class="row svelte-1tx7ah8"><button type="button" title="Open reference" aria-label="Open reference" class="svelte-1tx7ah8"><span class="material-icons-outlined svelte-1tx7ah8" aria-hidden="true">file_open</span></button> <input aria-label="Reference label" placeholder="Label" class="svelte-1tx7ah8"/> <select aria-label="Reference kind" class="svelte-1tx7ah8"></select> <input class="path svelte-1tx7ah8" aria-label="Relative path or URL" placeholder="../docs/document.md" spellcheck="false"/> <button class="remove svelte-1tx7ah8" type="button" title="Remove reference (keeps the file)" aria-label="Remove reference"><span class="material-icons-outlined svelte-1tx7ah8" aria-hidden="true">delete</span></button></div>`);
var root_5 = from_html(`<p class="error svelte-1tx7ah8" role="alert"> </p>`);
var root_1$6 = from_html(`<div class="references svelte-1tx7ah8"><p class="svelte-1tx7ah8">Paths are relative to the active .sys.blu file. Web URLs are also supported.</p> <div class="rows svelte-1tx7ah8"><!> <!></div> <!> <div class="actions svelte-1tx7ah8"><button type="button" class="svelte-1tx7ah8">Cancel</button><button type="button" class="svelte-1tx7ah8">Save</button></div></div>`);
...
function Project_references($$anchor, $$props) {
	...
	let box = mutable_source({ div: null, title: 'Project references' });
	let references = mutable_source([]);
	let error = mutable_source('');
	let request = mutable_source(null);
	...
	function add() {
		set(references, [
			...get$1(references),
			{ label: '', kind: 'documentation', target: '' }
		]);
	}

	function save() {
		const clean = get$1(references).map((reference) => ({
			...reference,
		...
		get$1(request).ok?.(clean);

		return true;
	}

	function accept() {
		if (save()) get$1(box).hide();
	}

	function cancel() {
		get$1(box).hide();
		get$1(request).cancel?.();
	}

	const handlers = {
		onProjectReferences(payload) {
			set(request, payload);
			set(references, structuredClone(payload.references ?? []));
			set(error, '');
			mutate(box, get$1(box).title = 'Project references');
			mutate(box, get$1(box).largeTitle = true);
			mutate(box, get$1(box).ok = save);
			mutate(box, get$1(box).cancel = payload.cancel);
			mutate(box, get$1(box).add = add);
			get$1(box).show(payload.pos);
		}
	};
		...
		children: ($$anchor, $$slotProps) => {
...

			{
				...
				if_block(node, ($$render) => {
					if ((
						get$1(references),
						untrack(() => get$1(references).length === 0)
					)) $$render(consequent);
				});
			}
			...
			each(node_1, 1, () => get$1(references), index, ($$anchor, reference, index$1) => {
				var div_2 = root_3$2();
				...
				each(select, 5, () => kinds, index, ($$anchor, kind) => {
					var option = root_4();
					...
					template_effect(() => {
						set_text(text, get$1(kind));

						if (option_value !== (option_value = get$1(kind))) {
							option.value = (option.__value = get$1(kind)) ?? '';
						}
					});
				...
				template_effect(($0) => button.disabled = $0, [
					() => (
						get$1(reference),
						untrack(() => !get$1(reference).target?.trim())
					)
				]);

				event('click', button, () => get$1(request).open?.(get$1(reference)));

				bind_value(input, () => get$1(reference).label, ($$value) => (
					get$1(reference).label = $$value,
					invalidate_inner_signals(() => (get$1(references)))
				));

				bind_select_value(select, () => get$1(reference).kind, ($$value) => (
					get$1(reference).kind = $$value,
					invalidate_inner_signals(() => (get$1(references)))
				));

				bind_value(input_1, () => get$1(reference).target, ($$value) => (
					get$1(reference).target = $$value,
					invalidate_inner_signals(() => (get$1(references)))
				));

				event('click', button_1, () => set(references, get$1(references).filter((_, i) => i !== index$1)));
				append($$anchor, div_2);
			});
			...
			event('click', button_2, cancel);
			event('click', button_3, accept);
			append($$anchor, div);
		},
...
var root_1$5 = from_html(`<div class="menu-item svelte-1gqilec"><i class="material-icons-outlined icon svelte-1gqilec"> </i> <div class="tooltip svelte-1gqilec"> </div></div>`);
var root$6 = from_html(`<div class="menu svelte-1gqilec"></div>`);
...
const ProjectReferencesFactory = getFactory(Project_references);
