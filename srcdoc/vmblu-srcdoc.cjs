...
function findTransmissions(sourceFile, filePath, nodeMap) {
    ...
    sourceFile.getDescendantsOfKind(tsMorph.SyntaxKind.CallExpression).forEach(node => {
        const expr = node.getExpression();

        // check
        if (expr.getKind() !== tsMorph.SyntaxKind.PropertyAccessExpression) return
        ...
        if (args.length === 0 || !args[0].isKind(tsMorph.SyntaxKind.StringLiteral)) return;

        const pin = args[0].getLiteralText();

        // Try to infer the class context of the tx.send call
        const method = node.getFirstAncestorByKind(tsMorph.SyntaxKind.MethodDeclaration);
        const classDecl = method?.getFirstAncestorByKind(tsMorph.SyntaxKind.ClassDeclaration) ?? node.getFirstAncestorByKind(tsMorph.SyntaxKind.ClassDeclaration);
...
//# sourceMappingURL=vmblu-srcdoc.cjs.map
