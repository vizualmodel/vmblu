...
test('init creates schema 1.10.1 model and visualization headers', async () => {
  ...
  try {
    ...
    assert.equal(model.header.version, '1.10.1');
    assert.equal(model.header.description, undefined);
    assert.equal(visual.header.version, '1.10.1');
