...
test('init creates schema 0.10.0 model and visualization headers', async () => {
  ...
  try {
    ...
    assert.equal(model.header.version, '0.10.0');
    assert.equal(model.header.description, undefined);
    assert.equal(visual.header.version, '0.10.0');
