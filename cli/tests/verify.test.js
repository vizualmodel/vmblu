...
test('verify accepts current generated artifacts and reports stale output', async () => {
  ...
  try {
    ...
    await writeFile(path.join(dir, 'sample.src.prf'), JSON.stringify({version: '1.12.1', provenance: profile, entries: []}))
    ...
    await writeFile(path.join(dir, 'sample.src.prf'), JSON.stringify({version: '1.12.1', provenance: staleProfile, entries: []}))
