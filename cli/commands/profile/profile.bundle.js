...
function defaultVizPath(modelArl) {
    const split$1 = split(modelArl.getPath());
    return split$1.name + (split$1.kind ?? '') + '.viz'
}
...
const ANY_PAYLOAD_SCHEMA = Object.freeze({type: 'object'});

function schemaFromPinContract(pin, vmbluTypes = null, direction = 'request') {
...
function schemaFromVmbluType(typeName, vmbluTypes = null, ancestors = new Set()) {
...
function hasReplyContract(pin) {
    const payload = pin?.contract?.payload;
    return !!(payload && typeof payload === 'object' && payload.reply)
}

function makeCapabilityId(nodePath, name) {
...
function capabilityTitle(value) {
    return String(value ?? '')
        .replace(/[-_.]+/g, ' ')
...
const CapabilityHandling = {

makeCapabilityObject(root = null) {
    const appName = root?.name || this.raw?.root?.name || getSplit(this.getArl()?.getPath?.() ?? '').name || '';
...
`;
    ...
    JSRuntimeOptions(runtime, node, srcArl, provenance) {
        ...
        if (this.isSafetyRuntime(runtime) && runtimeSettings) {
            const modelPath = normalizeSeparators(this.getArl().getFullPath());
...
PromptRepo.prototype = {
    ...
    getPath(refArl) {
        if (!this.arl || this.pathKind === Kind.Empty) return ''
    ...
    async hydratePromptRepos(raw = this.raw) {
        ...
        const hydrateNode = async (node) => {
            ...
            if (node.promptRepo?.arl) {
                ...
                if (parsed) {
                    node.prompt = normalizePrompt$1(parsed.prompt);
...
function resolvePromptRepoArl(promptRepo, refArl) {
    ...
    const arl = normalizeSeparators(promptRepo.arl);
    return (promptRepo.pathKind === Kind.Absolute || getKind(arl) === Kind.Absolute)
        ? refArl.resolve(arl)
        : refArl.resolve(arl)
}
...
function normalizePrompt$1(value) {
...
function getNodePromptDocument(node) {
...
function CableTack(cable, wid = null) {
...
const cableCxMenu = cm$2;


function cableHighLight() {
	cm$2.doEdit('cableHighlight',{cable: cm$2.cable});
}

function straightConnections() {
	cm$2.doEdit('cableStraightConnections', {cable: cm$2.cable});
}

function disconnect$1() {
	cm$2.doEdit('cableDisconnect',{cable: cm$2.cable});
}
...
function convertToRoutes() {
	cm$2.doEdit('cableToRoutes',{view: cm$2.view, cable: cm$2.cable});
}
...
function NodePrompts({prompt=null, repository=null}={}) {
...
function TestRepo(arl=null, pathKind=Kind.Empty, readOnly=false) {
    this.arl = arl;
    this.pathKind = pathKind;
...
TestRepo.prototype = {
    ...
    getPath(refArl) {
        if (!this.arl || this.pathKind === Kind.Empty) return ''
        if (!refArl) return this.arl.getPath()
        return this.pathKind === Kind.Absolute
            ? this.arl.getPath()
            : relative(this.arl.getFullPath(), refArl.getFullPath())
    },
...
function resolveTestRepo(raw, refArl, options) {
...
function Cable(from = {x:0, y:0}, uid = null, floating = false) {
...
Cable.prototype = {
    ...
    addTack(route) {
        ...
        const tackAtStart = route.from == null;
        const endpointZone = this.routeEndpointZone(route, tackAtStart);
        const newTack = new CableTack(this);
        newTack.setSelective(this.defaultTackSelectivity(other));
        newTack.setRoute(route);
        this.attachRouteEndpointTack(route, newTack, tackAtStart, endpointZone);
        this.tacks.push(newTack);
        return newTack
    },

    newTack(alias = null, selective = false) {
        const tack = new CableTack(this);
    ...
    copyTacks(newCable, newRoot) {
        ...
        for (const tack of this.tacks) {
            ...
            const newTack = new CableTack(newCable);
        ...
        for(const tack of this.tacks) {
            ...
            if (a.x == b.x) {
                ...
                if (other.rect.y > a.y && other.rect.y < b.y) {
                    ...
                    route.wire = canonicalOrthogonalWire(route.wire);
                    if (route.wire.length === 1) route.wire.push({...route.wire[0]});
                }
            }
        }
    },
        ...
        for (const source of sources) {
            for (const target of targets) {
                ...
                plans.push({
                    ...
                    to: target.getOther(),
                    wire: this.individualRouteWire(source, target)
                });
            }
        }
        return plans
...
function runtimeSettingsForSave(dx) {
...
function splitRouteWireForCable(routeWire, inset = style.cable.extraLength) {
...
function resolveEntrypoint(inputPath) {
  if (!inputPath) {
    throw new Error('resolveEntrypoint: inputPath is required');
  }

  const absoluteInputPath = path.resolve(inputPath);
  if (!fs$1.existsSync(absoluteInputPath) || !fs$1.statSync(absoluteInputPath).isFile()) {
  ...
  if (!fs$1.existsSync(modelPath) || !fs$1.statSync(modelPath).isFile()) {
