...
Vmblu supports the Model Context Protocol (MCP). An LLM can communicate with a vmblu application through a generated MCP tool file. MCP exposure is explicit: only handlers that are listed in a file-level exported `mcp` registry are exposed.

The recommended pattern is to define the MCP metadata in the same source file as the handlers it describes. The registry is keyed by handler name. Each entry defines the external tool contract for that handler, such as the tool name, description and parameter schema.
...
export const mcp = {
  onMessage: {
    name: 'send_message',
    description: 'Send a message to the active conversation.',
    params: {
      text: {
        type: 'string',
...
// the handler javascript
function onMessage({text}) {
  ...
}
```

The registry must be a plain exported object literal so vmblu tooling can read it reliably. In this first version, keep the metadata declarative. The generated MCP file is a derived artifact produced together with the application, so the runtime scaffold and the exposed MCP tool surface stay in sync.

For backward compatibility, vmblu still recognizes the legacy `@mcp` JSDoc tag on a handler. If both forms are present, the exported `mcp` registry is authoritative.
